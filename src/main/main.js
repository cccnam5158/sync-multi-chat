...
function createWindow() {
    // Prefer ICO on Windows for taskbar/shortcut fidelity; fall back to PNG.
    // These files are generated by scripts/generate-icons.js into /build (gitignored).
    const generatedIcoIconPath = path.join(__dirname, '../../build/icon.ico');
    const generatedPngIconPath = path.join(__dirname, '../../build/icon.png');
    const windowIconPath =
        (process.platform === 'win32' && fs.existsSync(generatedIcoIconPath))
            ? generatedIcoIconPath
            : (fs.existsSync(generatedPngIconPath) ? generatedPngIconPath : undefined);
