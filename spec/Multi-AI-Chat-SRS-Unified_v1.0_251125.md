...
#### AUTH-006: 미로그인 상태 처리
**[State-driven]**  
While the user has not logged in to a service, the system shall display the service's native login page within the corresponding panel.
...
#### AUTH-008: 로그인 세션 저장
**[Event-driven]**  
When the user successfully logs in to a service within a panel, the system shall persist the session cookies in the associated Electron session partition to allow automatic re-login on subsequent launches.
...
**[Ubiquitous]**  
The system shall maintain a Selector Config file containing DOM selectors for each service's input field.
...
#### ERR-001: 네트워크 오류
**[Unwanted]**  
If a Service Panel fails to load due to network error (e.g., DNS error, HTTP error), then the system shall display an error indicator within that panel with a "Reload" or "다시 시도" control for manual retry.
...
#### ERR-003: 크래시 복구
**[Event-Driven]**  
When the application is restarted after an unexpected crash, the system shall restore the previous window size and layout configuration.
