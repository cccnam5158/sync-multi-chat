...
**ë¬¸ì„œ ë²„ì „**: 0.10.0 (Session Persistence, WebView URL Bar, 3x1 Layout, External Link Handling)  
**?‘ì„±??*: 2025-12-09  
...
| ?©ì–´ | ?•ì˜ |
|------|------|
| MAPB | Sync Multi Chat (ë³?Electron ?? |
...
- 1x3 (Vertical Stack) - 1 column Ã— 3 rows
- 3x1 (Horizontal Split) - 3 columns Ã— 1 row
- 4x1 (Horizontal Split) - 4 columns Ã— 1 row, 4ê°??´ìƒ ?œë¹„???œì„±????ê¸°ë³¸ê°?- 2x2 (Grid Layout) - 2 columns Ã— 2 rows

The layout buttons shall be displayed in the following order (left to right): 1x3, 3x1, 4x1, 2x2.

#### LAYOUT-002-1: ?ˆì´?„ì›ƒ ë²„íŠ¼ ë¹„í™œ?±í™” ì¡°ê±´
**[State-Driven]**  
...
The system shall persist user settings in a JSON configuration file located at `%APPDATA%/sync-multi-chat/config.json`.
...
#### URLBAR-005: ?¸ë? ë¸Œë¼?°ì?ë¡??´ê¸°
**[Event-Driven]**
When the user clicks the Chrome icon (?Œ) in the URL bar, the system shall open the current URL in the system's default external browser using `shell.openExternal()`.

---

### 4.17 ?¸ë? ë§í¬ ì²˜ë¦¬ (EXTLINK)

#### EXTLINK-001: ?¸ë? ë§í¬ ê°ì?
**[Event-Driven]**
...
#### EXTLINK-002: ?œë¹„?¤ë³„ ?ˆìš© ?„ë©”??**[Ubiquitous]**
