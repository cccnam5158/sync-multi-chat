...
-   **자격 증명 미저장**: 비밀번호는 앱에 저장되지 않습니다. 공식 웹 UI 또는 구독 전용 OAuth 등 앱에서 제공하는 방식으로 로그인합니다.
-   **앱 내 Google / OAuth** *(v0.10.0)*: 지원되는 흐름에서는 Google·OpenAI 인증을 서비스 웹뷰와 **같은 파티션**의 **모달 창**에서 처리해, 불필요한 외부 브라우저 의존을 줄입니다.
-   **외부 브라우저 로그인** *(v0.8.1)*: 여전히 외부 프로필이 필요할 때는 **Google Chrome** 또는, 없으면 **Microsoft Edge**(Chromium)로 쿠키 동기화가 가능합니다.
