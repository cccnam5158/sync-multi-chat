...
-   **커스텀 프롬프트 빌더** *(v0.7.0 신규)*: 시스템/전역/개별 변수로 커스텀 프롬프트를 만들고 저장·관리합니다. 메인 입력창에서 슬래시 명령(`/`)으로 저장된 프롬프트를 불러와 인라인 변수 편집이 가능합니다. *(v0.8.0)* 프롬프트 빌더 및 메인 프롬프트 미리보기에서 **Mermaid** 다이어그램, **코드 블록**(문법 강조), **LaTeX** 수식 라이브 프리뷰 지원.
...
1.  최신 설치 파일을 다운로드합니다: [Sync-Multi-Chat-Setup-0.8.0.exe](https://github.com/cccnam5158/sync-multi-chat/releases/download/v0.8.0/Sync-Multi-Chat-Setup-0.8.0.exe)
