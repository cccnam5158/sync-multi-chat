...
-   **カスタムプロンプトビルダー** *(v0.7.0新機能)*: システム/グローバル/ローカル変数でカスタムプロンプトを作成・保存・管理。メイン入力でスラッシュコマンド（`/`）から保存プロンプトを挿入し、インライン変数編集が可能です。
...
1.  最新のインストーラーをダウンロードします: [Sync-Multi-Chat-Setup-0.7.0.exe](https://github.com/cccnam5158/sync-multi-chat/releases/download/v0.7.0/Sync-Multi-Chat-Setup-0.7.0.exe)
